...
// Server2 handoff briefing facts arrive as { label, value } where label is a
...
// ---------- handoff reason ----------
// Server1/Server2 may attach machine reasons like
...
const REASON_PREFIX_LABELS: Record<string, string> = {
  ...
  policy_gate: "回复策略校验未通过，已转交人工处理",
  policy_gate_after_tool: "回复策略校验未通过，已转交人工处理",
...
export function reasonLabel(reason: string | undefined | null): string {
  ...
  const prefix = (trimmed.split(":")[0] ?? trimmed).trim().toLowerCase();
...
const EVENT_LABELS: Record<string, string> = {
  ...
  "reply_policy.published": "发布策略",
  "reply_policy.draft_created": "创建策略草稿",
  "reply_policy.draft_updated": "修改策略草稿",
  "user.created": "创建账号",
  "user.updated": "修改账号",
  "user.disabled": "禁用账号",
  "user.enabled": "启用账号",
