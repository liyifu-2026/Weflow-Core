...
async function loadDocuments() {
  ...
  error.value = "";
  try {
    documents.value = await listKnowledgeFiles(selectedBaseId.value, {
...
/** 跳转到「平台管理」模式，在 WeKnora 完整界面中深链当前知识库 */
      ...
      <template v-else>
        ...
        >
          ...
          <span class="wf-content-name">
            ...
            <span class="wf-muted">{{ documentSource(item) }}</span>
          </span>
