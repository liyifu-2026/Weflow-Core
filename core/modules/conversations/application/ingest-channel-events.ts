...
/**
 * 逐事件摄取（单条隔离）：此前整页一事务，一条坏事件（不支持的
 * kind / 缺 mediaRef / 时间戳非法 / 任意 DB 错误）回滚整页、cursor
 * 不前进，同一页无限重试导致所有会话入站全部停摆。现在每个事件
 * 独立事务：归一化或入库失败只跳过该事件并记 error 日志（原始事件
 * 在 Host 事件库永久可查），游标照常推进，管道不再冻结。
 */
async function ingestNormalizedEvents(
  ...
  for (const rawEvent of events) {
    ...
    try {
      event = toNormalizedChannelEvent(rawEvent);
    } catch (error) {
      logger.error(
        { err: error, eventId: rawEvent.eventId },
        "channel event normalization failed; skipped",
      );
      continue;
    }
    ...
    try {
      await db.transaction(async (transaction) => {
        await ingestNormalizedEvent(transaction, event, {
          source,
          logger,
          groupChatDeps: groupChatDeps,
          deferredGlobalPause,
        ...
        if (eventCursor !== null) {
          await upsertChannelCursor(transaction, source, eventCursor);
        }
      });
    } catch (error) {
      logger.error(
        ...
        "channel event ingest failed; skipped",
      );
    }
  ...
  // 页末把游标推进到页游标（兜底覆盖被跳过事件自身的游标）
  await db.transaction(async (transaction) => {
    await upsertChannelCursor(transaction, source, nextCursor);
  });
}
...
/** 事件自身游标（Host 单调分配）；非法时返回 null（由页游标兜底推进） */
function eventCursorValue(cursor: string): number | null {
  const value = Number(cursor);
  return Number.isSafeInteger(value) && value >= 0 ? value : null;
}

async function upsertChannelCursor(
  transaction: IngestTransaction,
  source: string,
  cursor: number,
): Promise<void> {
