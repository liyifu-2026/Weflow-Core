...
> **注（2026-09-09）**：本文基于 v1.0.6，库当前为 v1.2.0，最新行为以 `README.zh-CN.md` 为准。
...
群聊图片默认只下发缩略图（原图需在微信中点开大图才落地）。开启
`WECHAT_MEDIA_ORIGINAL_VIA_UI=1` 后，媒体解析器在「仅缩略图可用/两者都不可
用」时，由后台线程调用 `MediaDownloader.download_image_original()`——通过
UI 自动化点击微信图片并点「图片原始大小」触发官方下载原图，成功后缓存于
`<media_staging>/ui-original/<media_ref>/`，下次请求直接以原图返回。

约束：全局同一时刻至多 1 个 UI 下载任务（串行化，避免抢窗口）；每条
media_ref 至多尝试 2 次；UI 流程会短暂抢占微信窗口，人工操作微信时建议
关闭本开关。Core 侧「查看原图」入口（`/content/original` +
`upgrade-channel-image-originals`）无需任何改动，升级自动生效。
