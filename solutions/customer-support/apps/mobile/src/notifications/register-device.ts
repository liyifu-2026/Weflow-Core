 ...
 *
 * 可靠性：注册失败（网络抖动/后端暂不可用）时记录"待注册"状态，
 * 应用回到前台时自动重试，直到 Core 确认注册成功。
 * 修复：之前注册失败即静默放弃，设备表为空导致所有推送被跳过
 * （outbox 无设备可发，通知永远到不了手机）。
 */
...
/** 待重试注册标记（内存级；进程重启后由登录流程重新触发注册） */
let pendingRetry: (() => void) | null = null;
let appStateHooked = false;

/** 注册失败时安排前台自动重试（同一时刻只挂一个监听） */
function scheduleRetryOnForeground(retry: () => void): void {
  pendingRetry = retry;
  if (appStateHooked) return;
  appStateHooked = true;
  AppState.addEventListener("change", (state) => {
    if (state === "active" && pendingRetry) {
...
export async function registerPushDevice(
  ...
  try {
    ...
    // 网络抖动/后端暂不可用：回前台后自动重试，不让设备注册静默丢失
    scheduleRetryOnForeground(() => {
      void attempt().catch(() => scheduleRetryOnForeground(() => undefined));
    });
  }
}
